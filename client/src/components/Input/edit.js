...
export default function InputForm({pasienID}) {

  const dispatch = useDispatch('');
  const history = useHistory();
  const classes = useStyles();

  const pasien = useSelector((state) => Object.values(state.spesific));
    ...
    return async (dispatch) => {
      await apis
        ...
        .then((response) => {
          ...
          dispatch({
            type: actionTypes.FETCH_PASIEN_ID,
            payload: response.data,
          });
          history.push(
            `/data`
  ...
  console.log('props luar', pasien)

  useEffect(() => {
    dispatch(fetchDataByID(pasienID));
  }, [pasienID]);
  ...
  return (
    <>
      <Paper className={classes.Paper} elevation={3}>
        <Typography variant="h5">Edit Data Pasien</Typography>
        <hr></hr>
        <TextField
          InputLabelProps={{
            shrink: true,
